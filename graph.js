...
class Graph {
  ...
  traverseGraphDF(startVertexId) {
    ...
    const traverse = (startingVertexId) => {
      const startVertex = this.vertices.find(
        (vertex) => vertex.id === startingVertexId
      );
      const notVisitedAdjVertices = startVertex.adjacentVertices.filter(
  ...
  getShortestPath(startVertexId, endVertexId) {
    ...
    const sIndex = this.vertices.findIndex(
      (vertex) => vertex.id === startVertexId
    );
    const eIndex = this.vertices.findIndex(
      (vertex) => vertex.id === endVertexId
    );
    if (sIndex === -1 || eIndex === -1)
      throw new Error("No valid id's provided as arguments");
    ...
    const foundPath = shortestPaths.find(
      (path) => path[path.length - 1] === endVertexId
    );
    return foundPath;
  }
}
...
console.log(
  "For example, to get distance from id1 to id7, call graph.getShortestPath('id1', 'id7')"
);
